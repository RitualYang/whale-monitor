...
class ZanEthClient:
    ...
    async def _rpc(self, method: str, params: list[Any]) -> Any:
        ...
        res = await self.client.post(self.rpc_url, json=payload, timeout=12)
        res.raise_for_status()
        data = res.json()
        if data.get("error"):
            raise RuntimeError(f"ZAN ETH RPC error: {data['error']}")
        return data.get("result")

    async def get_latest_block_number(self) -> int:
        hex_block = await self._rpc("eth_blockNumber", [])
        return int(hex_block, 16)
